...
public class Personnage {
	...
	public Personnage(Terrain terrain,String nom,Inventaire inventaire, int ptA, int ptV,int x, int y) { //personnage
	...
	public void attaque() {
		for(int i = 0 ; i < env.getPerso().size(); i++) {
			if(	(((getX() - env.getPerso().get(i).getX()) <= getEpee().getPorte()) && ((getX() - env.getPerso().get(i).getX()) >= -getEpee().getPorte())) &&  (getY() == env.getPerso().get(i).getY()) ){
				if(env.getPerso().get(i) instanceof pnj) {
					System.out.println("enfin");
					attaque(env.getPerso().get(i));
					if(env.getPerso().get(i).getVie() ==0) {
						env.getPerso().remove(i);
					}
				}
			}
		}
